...
- 只读分享台：为简历生成自包含快照链接，无需服务端即可打开；链接固定生成时刻的模块范围与顺序，源简历后续改动不改写旧链接；同一简历可生成多份互不覆盖的分享记录，可逐条撤销；损坏、旧版或超长链接明确失败且不触碰本机数据。
- 本地持久化：简历、个人资料、分享记录、主题偏好和模板偏好存储在 localStorage。
- JSON 导入导出：支持完整工作区备份和恢复（含分享记录）。
